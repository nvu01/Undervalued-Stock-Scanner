...
All data transformation steps are done in **Main_L**. 
The source table of **Main_L** is retrieved using **Path** (a dynamic project folder path) and **FileName** parameter.
To switch to another sector's **Main_L** table, choose a different **FileName** parameter value corresponding to the desired sector.
```m
let
