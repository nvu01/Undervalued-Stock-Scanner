...
All data transformation steps are done in **Main_M**. 
The source table of **Main_M** is retrieved using **Path** (a dynamic project folder path) and **FileName** parameter.
To switch to another sector's **Main_M** table, choose a different **FileName** parameter value corresponding to the desired sector.
```m
let
    ...
    Mid = #"Grouped Rows"{[#"Market Cap Category"="Mid"]}[Table],
    #"Removed Columns1" = Table.RemoveColumns(Mid,{"Market Cap Category"})
in
    #"Removed Columns1"
```